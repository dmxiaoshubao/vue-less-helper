...
- 混入预览缩进会先按最小公共缩进归一化，再统一增加 2 个空格偏移。
- 混入颜色注释（代码块下方色块行）仅基于 mixin 顶层声明计算：
  - 同名 key 取最后一次声明的 value
  - 忽略嵌套块中的声明
  - 顶层没有颜色声明时，不渲染颜色注释行
- 支持多根工作区：
  - 每个工作区根目录独立索引
...
| mixin 预览缩进归一化 | 是 | 先归一化最小公共缩进，再统一 +2 空格。 |
| mixin 颜色注释仅使用顶层声明 | 是 | 嵌套声明不会参与颜色注释。 |
| 顶层重复 key 取值策略 | 是 | 取最后一次声明的 value。 |
| 顶层无颜色时是否展示颜色注释 | 否 | 不回退使用嵌套层颜色。 |
